...
Execute exactly the same as `offer` mode (read `modes/offer.md` for all blocks A-F + Block G Posting Legitimacy).
...
1. **Extract form questions**: Use Playwright to navigate to the form and take a snapshot. If they cannot be extracted, use generic questions.
...
Register in `data/applications.md` with all columns including Report and PDF marked with ✅.
